...
export class NumberInputEditComponent extends AbstractFieldLikeEditForm<number, NumberInputComponentData> {
  ...
  get maxNumOrInf() {
    return this.getStrictControlValue('max') && this.getStrictControlValue('maxNumber') ? this.getStrictControlValue<number>('maxNumber') : Infinity
  }
  get minNumOrNegInf() {
    return this.getStrictControlValue('min') && this.getStrictControlValue('minNumber') ? this.getStrictControlValue<number>('minNumber') : -Infinity
  }
      ...
      return value => {
        ...
        if (specIndex > -1) {
          const [before, after] = [
            ...
            this.getStrictControlValue<string>('formatter').substring(specIndex + 3,  this.getStrictControlValue<string>('formatter').length)
          ];
